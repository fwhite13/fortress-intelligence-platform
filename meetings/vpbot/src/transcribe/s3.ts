...
export class S3Service {
   ...
   */
  async uploadAudio(filePath: string, meetingId: string): Promise<string> {
   ...
   */
  async uploadWav(filePath: string, meetingId: string): Promise<string> {
    const fileContent = fs.readFileSync(filePath);
   ...
   */
  async saveJsonWithKey(jsonStr: string, key: string): Promise<string> {
    const command = new PutObjectCommand({
      Bucket: this.bucket,
      Key: key,
      Body: jsonStr,
  ...
  async downloadToFile(key: string, localPath: string): Promise<void> {
    ...
    const response = await this.client.send(command);
    if (!response.Body) throw new Error(`S3 key not found: ${key}`);
   ...
   */
  async saveTranscript(meetingId: string, transcriptJson: string): Promise<string> {
   ...
   * Save summary to S3
   */
  async saveSummary(meetingId: string, summary: string): Promise<string> {
   ...
   */
  async findFirstKey(prefix: string, extension?: string): Promise<string | null> {
