...
export class MeetingBot extends EventEmitter {
    ...
    async startRecording() {
        ...
        this.page.on('close', () => {
            ...
            if (this.isRecording) {
                this.stop('page-close-event').catch((err) => console.error('[Bot] Error stopping on page close event:', err));
            }
        });
        ...
        this.ffmpegProcess.on('exit', (code) => {
            ...
            if (elapsed < 5000) {
                ...
                if (code !== 0) {
                    reportStatus(this.meeting.id, 'recording_failed', { error: errorMsg }).catch(e => console.error('[Bot] Failed to report recording_failed status:', e));
                }
        ...
        this._hardTimeout = setTimeout(() => {
            ...
            this.stop('max-recording-timeout').catch((err) => console.error('[Bot] Error stopping after MAX_RECORDING_MINUTES:', err));
        }, maxMs);
            ...
            try {
                ...
                for (const text of endTexts) {
                    ...
                    if (found) {
                        ...
                        this.stop('meeting-ended-detected').catch((err) => console.error('[Bot] Error stopping after end detection:', err));
                        return;
                    }
                }
                ...
                if (page.isClosed()) {
                    ...
                    if (this._endPollInterval)
                        clearInterval(this._endPollInterval);
                    this._endPollInterval = null;
                    this.stop('page-closed').catch((err) => console.error('[Bot] Error stopping after page close:', err));
                    return;
                }
                        ...
                        if (isTeams && currentUrl !== meetingUrl) {
                            ...
                            if (isAboutBlank || isTeamsHome || isTeamsConversations) {
                                ...
                                if (this._endPollInterval)
                                    clearInterval(this._endPollInterval);
                                this._endPollInterval = null;
                                this.stop('navigation-away').catch((err) => console.error('[Bot] Error stopping after navigation away:', err));
                                return;
                            }
                        }
                    }
                    ...
                    if (count <= 1) {
                        ...
                        if (participantOneCount >= 5 && pastMinDuration) {
                            ...
                            if (this._endPollInterval)
                                clearInterval(this._endPollInterval);
                            this._endPollInterval = null;
                            this.stop('participant-count-timeout').catch((err) => console.error('[Bot] Error stopping after participant count timeout:', err));
                            return;
                        }
                    }
                    else {
                ...
                if (this._silenceStartTime !== null && pastMinDuration) {
                    ...
                    if (silenceDurationMs > 90_000 && aloneOrUnknown) {
                        ...
                        if (this._endPollInterval)
                            clearInterval(this._endPollInterval);
                        this._endPollInterval = null;
                        this.stop('audio-silence-timeout').catch((err) => console.error('[Bot] Error stopping after audio silence timeout:', err));
                        return;
                    }
                }
            }
