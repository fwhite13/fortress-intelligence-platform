...
exports.runDesignTask = runDesignTask;
...
async function runTaskWithEmit(params, emit) {
    for await (const chunk of (0, runner_js_1.runTask)(params)) {
        emit(chunk);
    }
}
async function runDesignTask(params, emit) {
    const { taskId, orgId, projectId, prompt, variantCount, deviceTarget, convertToBlazor, priorHtml, screenId } = params;
    // ── 1. Load brand context ─────────────────────────────────────────────
    emit({ type: 'step', text: 'Loading brand context...' });
    const brand = await (0, brandService_js_1.getBrandContext)(orgId);
    const brandBlock = (0, brandService_js_1.formatBrandContextForPrompt)(brand);
    // ── 2. Build system prompt ────────────────────────────────────────────
    const rawSystemPrompt = await promises_1.default.readFile(path_1.default.join(process.cwd(), 'agents/design/system-prompt.md'), 'utf8');
    ...
    await runTaskWithEmit({ taskId, userId: params.userId, userEmail: params.userEmail,
        prompt: userPrompt, workingDir, maxBudgetUsd: 0.50, maxTurns: 8,
        systemPromptOverride: systemPrompt }, emit);
    ...
    const sid = screenId ?? crypto_1.default.randomUUID();
    ...
    const htmlContent = await promises_1.default.readFile(generated, 'utf8');
    ...
    const downloadUrl = await (0, s3_request_presigner_1.getSignedUrl)(s3, new client_s3_1.GetObjectCommand({ Bucket: S3_BUCKET, Key: s3Key }), { expiresIn: 3600 });
    ...
    if (convertToBlazor) {
        ...
        await runBlazorConversion(taskId, htmlContent, orgId, projectId, sid, version, emit);
    }
}
// ── Variant generation ────────────────────────────────────────────────────
async function runVariantTask(params, systemPrompt, _brand, emit) {
    ...
    const results = await Promise.allSettled(variantInstructions.map(async (variant, i) => {
        if (i > 0)
            await new Promise(r => setTimeout(r, i * 500)); // 0ms, 500ms, 1000ms
        const varWorkingDir = `/tmp/cowork-${taskId}-${variant.suffix}`;
        await promises_1.default.mkdir(varWorkingDir, { recursive: true });
        const varPrompt = [
        ...
        const chunks = [];
        await runTaskWithEmit({ taskId: `${taskId}-${variant.suffix}`, userId: params.userId,
            userEmail: params.userEmail, prompt: varPrompt,
            workingDir: varWorkingDir, maxBudgetUsd: 0.35, maxTurns: 6,
            systemPromptOverride: systemPrompt }, (chunk) => chunks.push(chunk));
...
// ── Blazor conversion ─────────────────────────────────────────────────────
async function runBlazorConversion(taskId, htmlContent, orgId, projectId, screenId, version, emit) {
    ...
    await promises_1.default.mkdir(convWorkingDir, { recursive: true });
    await runTaskWithEmit({ taskId: `${taskId}-blazor`, userId: 'system', userEmail: 'system',
    ...
    (chunk) => {
        if (chunk.type !== 'step' && chunk.type !== 'tool_call')
            return; // suppress verbose
    });
    const razorFile = path_1.default.join(convWorkingDir, 'component.razor');
    try {
        const razorContent = await promises_1.default.readFile(razorFile, 'utf8');
...
// ── Helpers ───────────────────────────────────────────────────────────────
async function findGeneratedHtml(dir) {
    try {
        const files = await promises_1.default.readdir(dir);
        const html = files.find(f => f.endsWith('.html'));
        return html ? path_1.default.join(dir, html) : null;
    }
    catch {
        return null;
    }
}
async function getNextVersion(orgId, projectId, screenId) {
    const { getRedis } = await import('../../services/taskStore.js');
    const redis = await getRedis();
