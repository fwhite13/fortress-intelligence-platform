...
app.post('/convert-xlsx', authMiddleware, async (req, res) => {
    const { artifactId, s3Key, userId, outputBucket } = req.body || {};
    if (!artifactId || !s3Key || !userId || !outputBucket) {
        return res.status(400).json({ error: 'Missing required fields: artifactId, s3Key, userId, outputBucket' });
    }
    ...
    const presizedPath = `/tmp/${artifactId}-pagesized.xlsx`;
    const pdfPath = `/tmp/${artifactId}.pdf`;

    try {
        // Step 1: Download XLSX from S3
        console.log(`[convert-xlsx] Downloading s3://${outputBucket}/${s3Key} → ${xlsxPath}`);
        const getCmd = new GetObjectCommand({ Bucket: outputBucket, Key: s3Key });
        const s3Resp = await s3.send(getCmd);
        await new Promise((resolve, reject) => {
        ...
        // Step 2: Pre-size with ExcelJS
        console.log(`[convert-xlsx] Pre-sizing workbook for ${artifactId}`);
        const sheetNames = await presizeWorkbook(xlsxPath, presizedPath);
        console.log(`[convert-xlsx] Sheet names: ${sheetNames.join(', ')}`);

        // Step 3: Convert with LibreOffice
        console.log(`[convert-xlsx] Running LibreOffice conversion for ${artifactId}`);
        const actualPdfPath = `/tmp/${artifactId}-pagesized.pdf`;
        await new Promise((resolve, reject) => {
            const lo = spawn('libreoffice', [
                '--headless',
                `--env:UserInstallation=file:///tmp/lo-profile-${artifactId}`,
        ...
        console.log(`[convert-xlsx] Done: ${artifactId}`);
        return res.json({ status: 'done', previewS3Key, sheetNames });
    } catch (err) {
        if (err.isTimeout) {
            console.error(`[convert-xlsx] Timeout converting ${artifactId}`);
            return res.status(504).json({ error: 'Conversion timed out' });
        }
        console.error(`[convert-xlsx] Error converting ${artifactId}:`, err.message);
        return res.status(500).json({ error: 'Conversion failed', detail: err.message });
    } finally {
