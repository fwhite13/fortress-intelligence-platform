                ...
                for await (const event of response.stream) {
                    if (event.contentBlockStart?.start?.toolUse) {
                        ...
                        if (toolUseAccumulator.name === 'list_workspace_files') {
                            sendEvent({ type: 'chip', content: getBedrockChipText(toolUseAccumulator.name, toolInput), toolName: toolUseAccumulator.name });
                            try {
...
// ADO#4569: PPTX → PDF conversion endpoint
app.post('/convert-pptx-preview', async (req, res) => {
    const { artifactId, s3Key, userId, previewS3Key } = req.body;
    if (!artifactId || !s3Key || !userId) {
        return res.status(400).json({ error: 'artifactId, s3Key, and userId are required' });
    }

    const outPreviewKey = previewS3Key || `workspaces/${userId}/previews/${artifactId}.pdf`;

    // Cache check: if previewS3Key provided, verify it exists in S3
    if (previewS3Key) {
        try {
    ...
    const tmpDir = `/tmp/pptx-${artifactId}`;
    const tmpInput = `${tmpDir}/input.pptx`;
    ...
    try {
        ...
        const s3Resp = await s3Client.send(new GetObjectCommand({ Bucket: S3_BUCKET, Key: s3Key }));
        const chunks = [];
        ...
        await new Promise((resolve, reject) => {
            ...
            proc.on('close', code => {
                if (code !== 0) reject(new Error(`LibreOffice exit ${code}: ${stderr}`));
                else resolve();
            });
        });
